...
const voice = z.object({ mode: z.enum(['local', 'online']), microphoneDeviceId: z.string().max(512).optional(), autoStop: boolean, pauseDurationMs: z.number().int().min(600).max(3000), maxRecordingSeconds: z.number().int().min(15).max(300), noiseSuppression: boolean, echoCancellation: boolean, autoGainControl: boolean })
