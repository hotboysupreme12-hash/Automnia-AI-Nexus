...
import { createBufferedJsonStorage } from './bufferedJsonStorage'
...
import {
  ...
  type NexusPersistedPayload,
} from './nexusPersistence'
import { redactDiagnosticText, safeDiagnosticPayload } from '../utils/diagnosticRedaction'
import { activityTypeForOperationalText, compactLine, redactActivityText, safeActivityPayload, severityForActivity, surfaceForActivity } from './runtimeActivity'
import type {
  AgentMessage,
  AgentMessageKind,
  AgentActivityEvent,
...
const bufferedNexusStorage = createBufferedJsonStorage<NexusPersistedPayload>(
  () => localStorage,
  () => {
    const now = Date.now()
    if (now - lastStorageWarningAt < 60_000) return
    lastStorageWarningAt = now
    console.warn('Automnia could not save local preferences. The previous saved copy is preserved.')
...
if (typeof window !== 'undefined') {
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden') bufferedNexusStorage.flush()
  })
}
...
interface NexusState extends NexusAgentConfigState, NexusMissionState, NexusUiState, NexusRuntimeProjectionState, NexusCommandConsoleResponseState, NexusCoordinationState {
  ...
  editQueuedCommandConsoleFollowup: (responseId: string, prompt: string) => boolean
  queuedCommandConsolePrompt: (responseId: string) => string | null
  moveQueuedCommandConsoleFollowup: (responseId: string, direction: -1 | 1) => boolean
  ingestClawTalkConsoleEvent: (event: ClawTalkConsoleEvent) => void
  clearAgentResponses: () => Promise<{ ok: boolean; message: string }>
  ...
  saveAgentPolicies: (agentId: string) => Promise<{ ok: boolean; message: string }>
...
export const useNexusStore = create<NexusState>()(
  persist(
    (set, get) => {
      ...
      type QueuedCommandConsoleFollowup = CommandConsoleQueuedFollowup<PromptRunOptions> & { userPrompt: string; rebuildPrompt: (prompt: string) => string }
      ...
      const refreshQueuedCommandConsolePositions = (agentId: string) => {
        ...
        const positionById = new Map(queue.map((entry, index) => [entry.id, index + 1]))
      ...
      const queueCommandConsoleFollowups = (
        ...
        sourceLabel: string,
        promptForAgent?: (agent: OpenClawAgent, index: number, prompt: string) => string,
      ) => {
        ...
        for (const [index, agent] of agentsToQueue.entries()) {
          ...
          const rebuildPrompt = (prompt: string) => promptForAgent?.(agent, index, prompt) || prompt
          const runPrompt = rebuildPrompt(normalizedPrompt)
      ...
      const publishClawTalkConsoleEvent = (frame: ClawTalkConsoleEvent) => {
            ...
            return {
              ...
              operationStates: {
                ...
                [agentId]: {
                  ...
                  logStream: eventName === 'delta' ? prev.logStream : [
                    `${isTerminal ? ok ? 'OK' : 'ERR' : 'RUN'} ${new Date(ts).toLocaleTimeString()} ${(response || progressText || 'ClawTalk activity').slice(0, 180)}`,
                    ...prev.logStream,
                  ].slice(0, 28),
                  uptimeMs: prev.uptimeMs + Math.max(0, elapsedMs - (existing?.durationMs || 0)),
                },
              },
            }
          })
      }
      const clawTalkBatch = createTextEventBatcher(publishClawTalkConsoleEvent, (frame) =>
        frame.clawTalkRunId?.trim() || frame.runId?.trim() || frame.sessionKey?.trim() || frame.id?.trim() || '')

      let consoleClearPromise: Promise<{ ok: boolean; message: string }> | null = null
      const clearConsole = (party: boolean) => {
        if (consoleClearPromise) return consoleClearPromise
        clawTalkBatch.flush()
        const responseIds = new Set(get().agentResponses.map((entry) => entry.id))
        const partyBefore = get().activePartyIds.join('|')
        clearQueuedCommandConsoleFollowups()
        resetCommandConsoleRuntimeState()
        consoleClearPromise = (async () => {
          try {
            const result = await clearAgentTurnSessions()
            if (!result.ok) throw new Error(apiErrorMessage(result.error))
            set((state) => ({
              agentResponses: state.agentResponses.filter((entry) => !responseIds.has(entry.id)),
              ...(party && state.activePartyIds.join('|') === partyBefore
                ? { activePartyIds: [], confirmedPartyIds: [], selectedAgentId: null, selectedAgentIds: [] }
                : {}),
            }))
            const outcome = { ok: true, message: party ? 'Party and console sessions cleared.' : 'Console responses and AI sessions cleared.' }
            if (typeof window !== 'undefined') window.dispatchEvent(new CustomEvent('automnia:console-clear-result', { detail: { ...outcome, party } }))
            return outcome
          } catch (error) {
            const outcome = { ok: false, message: `AI sessions could not be cleared. Your conversation history is preserved. ${redactDiagnosticText(String(error), 180)}` }
            if (typeof window !== 'undefined') window.dispatchEvent(new CustomEvent('automnia:console-clear-result', { detail: { ...outcome, party } }))
            return outcome
          } finally { consoleClearPromise = null }
        })()
        return consoleClearPromise
      }
      ...
      return {
        ...
        queuedCommandConsolePrompt: (responseId) => {
          for (const queue of queuedCommandConsoleFollowups.values()) {
            const task = queue.find((entry) => entry.id === responseId)
            if (task) return task.userPrompt
          }
          return null
        },
        editQueuedCommandConsoleFollowup: (responseId, prompt) => {
          const normalized = normalizeOperatorPrompt(prompt)
          if (!normalized || normalized.length > 100_000) return false
          for (const queue of queuedCommandConsoleFollowups.values()) {
            const task = queue.find((entry) => entry.id === responseId)
            if (!task) continue
            task.prompt = task.rebuildPrompt(normalized)
            task.userPrompt = normalized
            task.options.displayPrompt = normalized
            task.visiblePrompt = promptWithAttachmentNames(normalized, task.options.attachments)
            updateQueuedCommandConsoleResponse(task.id, { prompt: task.visiblePrompt })
            return true
          }
          return false
        },
        moveQueuedCommandConsoleFollowup: (responseId, direction) => {
          if (direction !== -1 && direction !== 1) return false
          for (const [agentId, queue] of queuedCommandConsoleFollowups) {
            const index = queue.findIndex((entry) => entry.id === responseId)
            if (index < 0) continue
            const target = index + direction
            if (target < 0 || target >= queue.length) return false
            ;[queue[index], queue[target]] = [queue[target], queue[index]]
            refreshQueuedCommandConsolePositions(agentId)
            return true
          }
          return false
        },
        ingestClawTalkConsoleEvent: (frame) => {
          ...
          clawTalkBatch.push(frame.event === 'delta' ? { ...frame, text: rawText } : frame)
        },
        clearAgentResponses: () => clearConsole(false),
        clearAll: () => clearConsole(true),
        ...
        saveAgentPolicies: async (aid) => {
          const agent = get().agents.find((entry) => entry.id === aid)
          if (!agent) return { ok: false, message: 'Agent no longer exists.' }
          // Supersede queued per-field writes before saving the combined snapshot.
          const scopes = [
            ['heartbeat', heartbeatConfigSaveTimers, heartbeatConfigSaveSeq],
            ['runtime', runtimePolicySaveTimers, runtimePolicySaveSeq],
          ] as const
          const versions = scopes.map(([scope, timers, sequences]) => {
            clearTimeout(timers.get(aid))
            timers.delete(aid)
            const seq = (sequences.get(aid) || 0) + 1
            sequences.set(aid, seq)
            reportAgentConfigSave(aid, scope, configSaveEntry('saving', 'Saving agent policies...', seq))
            return seq
          })
          const result = await saveAgentConfig(aid, {
            heartbeat: {
              tickIntervalMs: agent.heartbeat.tickIntervalMs,
              maxExecutionTimeMs: agent.heartbeat.maxExecutionTimeMs,
              idleTimeoutMs: agent.heartbeat.idleTimeoutMs,
              continuous: agent.heartbeat.continuous,
              recoveryMode: agent.heartbeat.recoveryMode,
            },
            runtime: {
              thinkingDefault: agent.runtimePolicy?.thinkingDefault,
              timeoutSeconds: agent.runtimePolicy?.timeoutSeconds,
              parallelPreferred: agent.runtimePolicy?.parallelPreferred,
              fastModeDefault: agent.runtimePolicy?.fastModeDefault,
            },
          })
          const message = result.ok ? 'Agent policies saved.' : apiErrorMessage(result.error)
          scopes.forEach(([scope, , sequences], index) => {
            if (sequences.get(aid) === versions[index]) {
              reportAgentConfigSave(aid, scope, configSaveEntry(result.ok ? 'saved' : 'failed', message, versions[index], result.requestId))
            }
          })
    ...
    },
    {
      ...
      storage: bufferedNexusStorage.storage,
