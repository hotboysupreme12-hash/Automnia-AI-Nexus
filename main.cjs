...
function existingManagedNpmBin() {
  if (!fs.existsSync(NPM_TOOLCHAIN_ROOT)) return ''
  return fs.readdirSync(NPM_TOOLCHAIN_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && /^node-v\d+\.\d+\.\d+-win-x64$/i.test(entry.name))
    .map((entry) => path.join(NPM_TOOLCHAIN_ROOT, entry.name))
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    // Tell the server to use Electron's Node binary (v22) for openclaw agent
    ...
    console.log('[dystopai] loading server inline:', serverEntry)

    // Load the server module inline — no child process
    require(serverEntry)
