...
const tempRoot = mkdtempSync(path.join(tmpdir(), 'dystopai-phase-k-desktop-'))
...
const env = {
  ...
  DYSTOPAI_ELECTRON_E2E_ASSERT_DESKTOP_BOOTSTRAP: '1',
  DYSTOPAI_ELECTRON_E2E_QUIT_AFTER_DESKTOP_BOOTSTRAP: '1',
...
try {
  ...
  logContent = await waitForLogPatterns(logPath, [
    ...
    /\[dystopai-e2e\] desktop-session-bootstrap-token-length:\d+/,
    /\[dystopai-e2e\] desktop-session-bootstrap-ok/,
