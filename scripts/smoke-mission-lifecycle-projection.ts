...
assert.match(serverSource, /app\.get\('\/api\/missions', async \(_req, res\) => \{\s*\n\s*return apiSuccess\(res, await buildMissionLifecycleProjection/)
assert.match(serverSource, /app\.get\('\/api\/missions\/projection'/)
assert.match(serverSource, /app\.get\('\/api\/missions\/:missionId\/lifecycle'/)
assert.match(serverSource, /Mission lifecycle not found/)
