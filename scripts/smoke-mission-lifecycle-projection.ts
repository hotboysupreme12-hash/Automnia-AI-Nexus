...
assert.match(missionReportServiceSource, /function missionFeedEventFromLifecycleEvent/)
assert.match(missionReportServiceSource, /function mergeMissionFeedEvents/)
assert.match(missionReportServiceSource, /async function listMissionRecordsForProjection/)
assert.match(missionReportServiceSource, /readMissionRecords<MissionRecordSnapshot>/)
assert.match(missionReportServiceSource, /async function buildMissionLifecycleProjection/)
assert.match(missionReportServiceSource, /source: 'memory\+ledger'/)
assert.match(serverSource, /const buildMissionLifecycleProjection = missionReportService\.buildMissionLifecycleProjection/)
...
assert.match(missionApiSource, /export type BackendMissionsPayload = \{/)
assert.match(missionApiSource, /projection\?: \{/)
assert.match(missionApiSource, /apiRequest<BackendMissionsPayload>\('\/api\/missions\/projection'\)/)
assert.match(storeSource, /from '..\/api\/missions'/)
...
assert.match(storeSource, /await fetchMissionProjection\(\)/)
