...
assertIncludes(server, 'AUTOMNIA_CREDITS_COMPACT_TOOL_ALLOWLIST', 'Automnia credits should use a bounded tool allowlist')
assertIncludes(server, "[AUTOMNIA_CREDITS_PROVIDER_ID]:", 'Automnia credits tool policy should be provider-scoped')
...
assertIncludes(agentConfigRoutes, "local.tools = normalizeAgentToolsConfig({ profile: 'full' })", 'Sandbox off clears restrictive per-agent tool filters')
assertIncludes(server, 'function unrestrictedAgentToolsConfig()', 'Control plane has one unrestricted host tool policy')
assertIncludes(server, "if (entry.sandbox?.mode === 'off') entry.tools = unrestrictedAgentToolsConfig()", 'Runtime config normalization repairs existing sandbox-off agents')
assertIncludes(server, "target.tools = local.sandbox.mode === 'off'", 'Global agent projection enforces full tools when sandbox is off')
assertIncludes(server, "if (local.sandbox.mode === 'off')", 'Runtime preflight repairs legacy sandbox-off tool restrictions')
