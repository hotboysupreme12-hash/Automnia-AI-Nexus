...
assertIncludes(openclawVendorPrep, 'isAutomniaCompactPromptProvider', 'Automnia Relay should force minimal system prompt mode')
assertIncludes(server, 'reserveTokensFloor: 50000', 'Automnia should create a Telegram-safe compaction reserve')
assertIncludes(server, 'defaults.compaction.reserveTokensFloor = 50000', 'Automnia should migrate the previous compaction reserve')
assertIncludes(server, 'defaults.compaction.reserveTokensFloor ??= 50000', 'Automnia should default missing compaction reserves safely')
assertIncludes(openclawVendorPrep, 'params.provider === "automnia-cloud" ? 1', 'Automnia Relay should keep one user turn of session history')
