...
assertIncludes(server, 'appendRuntimeRunLedger(openClawRunLedgerPayload(record), { mirrorJsonl: false })', 'Runtime run hot path writes SQLite-primary state')
...
assertIncludes(server, 'function isNodeDeprecationWarningLine', 'Gateway monitor filters Node deprecation warning noise')
assertIncludes(server, '\\[DEP\\d+\\]', 'Gateway monitor filters Node DEP warning codes')
assertIncludes(server, '--trace-deprecation', 'Gateway monitor filters Node trace-deprecation helper lines')
assertIncludes(server, 'function isGatewayFailoverDecisionNoise', 'Gateway monitor filters internal failover decision noise')
assertIncludes(server, 'model fallback decision', 'Gateway monitor suppresses model fallback decision chatter')
...
assertIncludes(uiSmoke, '!agentsTab.commandConsole.traceChipPresent', 'UI smoke verifies Command Console trace chip is absent')
assertIncludes(uiSmoke, '!agentsTab.commandConsole.evidencePreviewPresent', 'UI smoke verifies Command Console evidence preview is absent')
assertIncludes(uiSmoke, 'thinkingBodyPresent', 'UI smoke verifies inline Command Console thinking body')
assertIncludes(uiSmoke, '!agentsTab.commandConsole.gatewayAcceptedVisible', 'UI smoke verifies Gateway acceptance copy is replaced by inline thinking')
assertIncludes(uiSmoke, '!agentsTab.commandConsole.runTraceVisible', 'UI smoke verifies run id trace chip text is absent')
...
assertIncludes(routingHelpers, 'TELEGRAM_AGENT_ROUTING_PATCH_VERSION = 3', 'Telegram routing patch version')
...
assertIncludes(server, "TELEGRAM_REPAIR_SIGNATURE_VERSION = 'telegram-routing-repair:v3'", 'Telegram repair signature version')
