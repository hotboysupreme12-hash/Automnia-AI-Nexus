...
const finalOverrides = read('src/styles/automnia-theme/50-final-overrides.css')
...
const openclawVendorPrep = read('scripts/prepare-openclaw-vendor.cjs')
const relayTokenOptimization = read('infra/gcloud/service/tokenOptimization.js')
...
assertIncludes(openclawVendorPrep, 'ensureAutomniaRelayThoughtSignatureSupport', 'OpenClaw vendor prep should preserve Automnia Gemini tool-call signatures')
assertIncludes(openclawVendorPrep, 'isAutomniaGeminiRelay', 'OpenClaw vendor prep should identify the hosted Automnia Gemini route')
assertIncludes(openclawVendorPrep, 'ensureAutomniaRelayRetrySafetySupport', 'OpenClaw vendor prep should install Automnia Relay retry safety')
assertIncludes(openclawVendorPrep, 'maxRetries: 0', 'Automnia Relay should disable duplicate SDK retries')
assertIncludes(openclawVendorPrep, 'Idempotency-Key', 'Automnia Relay requests should carry a stable idempotency key')
...
assertIncludes(server, 'defaults.compaction.reserveTokensFloor = AUTOMNIA_COMPACTION_RESERVE_TOKENS', 'Automnia should migrate the previous compaction reserve')
assertIncludes(server, 'defaults.compaction.reserveTokensFloor ??= AUTOMNIA_COMPACTION_RESERVE_TOKENS', 'Automnia should default missing compaction reserves safely')
assertIncludes(server, "midTurnPrecheck: { enabled: true }", 'Automnia should precheck tool-loop context pressure')
...
assertNotIncludes(server, 'MAX_AUTOMNIA_RELAY_CONVERSATION_MESSAGES', 'direct provider history must not have a larger Automnia-only exception')
assertIncludes(openclawVendorPrep, 'Math.min(4, Math.max(1, getHistoryLimitFromSessionKey', 'every model should keep a bounded rolling session history')
assertIncludes(server, 'removeGeneratedHostedToolAllowlist', 'Automnia credits should normalize generated hosted tool policy')
assertIncludes(server, 'providers[AUTOMNIA_CREDITS_PROVIDER_ID]', 'Automnia credits tool policy should be provider-scoped')
assertIncludes(server, 'AUTOMNIA_CREDITS_COMPACT_TOOL_RESULT_MAX_CHARS', 'Automnia credits should cap live tool results')
assertIncludes(relayTokenOptimization, 'for (let pass = 0; JSON.stringify(compacted).length > inputLimitChars', 'relay compaction should enforce the aggregate input budget iteratively')
assertIncludes(relayTokenOptimization, 'const optionalLimit = Math.max(0, maxSchemaProperties - requiredNames.length)', 'relay schema compaction should preserve required fields before optional fields')
assertIncludes(server, 'DIRECT_PROVIDER_REQUEST_TIMEOUT_MS', 'direct provider requests should have a bounded deadline')
assertIncludes(server, 'Automnia credits compact runtime context:', 'Automnia command turns should use compact identity context')
assertIncludes(routingHelpers, 'Automnia credits compact identity (authoritative for this turn):', 'Telegram Automnia turns should use compact identity context')
assertIncludes(routingHelpers, 'readTelegramAgentMemorySnippet', 'Telegram compact context should include a bounded memory snippet')
...
assertIncludes(server, 'appendGatewayLogEntry: (entry) => {', 'Gateway log ledger append uses the durable store boundary')
assertIncludes(server, 'return runtimeLedgerStore.appendGatewayEvent(entry, { mirrorJsonl: false })', 'Gateway log ledger append uses SQLite-primary durable storage')
...
assertIncludes(gatewayLogService, '})).catch(() => undefined)', 'Gateway log hot path keeps durable writes off the initiating call stack')
...
assertIncludes(runtimeStatusService, 'readRuntimeGatewayLedgerSnapshot(48, { sqlite: false })', 'Runtime summary avoids synchronous Gateway log SQLite reads')
assertIncludes(runtimeStatusService, 'runtimeStatusPayloadTimedOut', 'Runtime status keeps timed-out refreshes single-flight')
...
assertIncludes(gatewayAgentTurnService, 'const gatewayMessage = isFreshSession', 'Command Console Gateway chat message composition')
assertIncludes(gatewayAgentTurnService, 'continuation?: boolean', 'continuation turns use the compact Gateway prompt contract')
assertIncludes(server, 'if (continuation) return composeAutomniaContinuationPrompt(message)', 'fresh turns retain the full prompt while continuations stay compact')
assertIncludes(server, 'Preserve ISO-8601 timestamps, UUIDs, and numeric measurements exactly', 'runtime prompts preserve exact machine-readable values')
...
assertIncludes(agentTurnRoutes, "const initialTransport = 'gateway-chat'", 'Hosted-credit turns use the Gateway transport')
...
assertIncludes(agentTurnRoutes, "writeSseEvent(res, 'heartbeat'", 'Command Console stream should keep quiet long turns alive')
...
assertIncludes(partyManagementRoutes, "local.sandbox = normalizeSandboxConfig({ mode: 'off', scope: 'agent', workspaceAccess: 'rw' })", 'Recruit API persists the explicit default execution policy')
...
assertIncludes(agentConfigRoutes, "local.tools = normalizeAgentToolsConfig({ profile: 'full', ...local.tools })", 'Sandbox off clears restrictive per-agent tool filters')
assertIncludes(server, 'fullAccessToolPolicy', 'Control plane has one unrestricted host tool policy')
assertIncludes(server, "if (entry.sandbox?.mode === 'off') entry.tools = normalizeAgentToolsConfig({ profile: 'full', ...entry.tools })", 'Runtime config normalization repairs existing sandbox-off agents')
assertIncludes(server, 'target.tools = normalizeAgentToolsConfig(local.tools)', 'Global agent projection normalizes tools when sandbox is off')
assertIncludes(server, "entry.sandbox = { mode: 'off', scope: 'agent', workspaceAccess: 'rw' }", 'Runtime preflight repairs legacy sandbox-off tool restrictions')
...
assertIncludes(commandConsole, 'lastSendAttemptRef.current === sendFingerprint', 'Command Console synchronously guards duplicate send attempts')
assertIncludes(commandConsole, 'const draftPrompt = prompt', 'Command Console sends one immutable draft snapshot')
...
assertIncludes(finalOverrides, '.dy-command-busy', 'Command Console running indicator styling')
...
assertIncludes(agentTurnStreamRoute, 'const hostedBrowserIntent = hostedCreditRoute', 'hosted browser turns should use the shared browser intent gate')
assertIncludes(agentTurnStreamRoute, 'const preflight = await checkBrowserPreflight()', 'hosted browser turns should run browser-service preflight')
assertIncludes(agentTurnStreamRoute, "const hostedInput = hostedBrowserIntent ? { ...parsed.data, thinking: 'off' as const } : parsed.data", 'hosted browser turns should use the browser-safe thinking mode')
...
assertRegex(gatewayProtocolDocs, /Side-effecting methods require (?:\*\*)?idempotency keys(?:\*\*)?(?: \(see schema\))?\./u, 'Gateway protocol idempotency requirement')
assertRegex(gatewayProtocolDocs, /Chat execution (?:still )?uses `chat\.history`, `chat\.send`, `chat\.abort`, and `chat\.inject`/u, 'Gateway protocol chat execution methods')
...
assertIncludes(gatewayChatService, "if (state === 'final' && isVisibleGatewayAssistantText", 'Gateway final event handler')
...
assertIncludes(server, 'CLAWTALK_ROUTING_PATCH_VERSION = 14', 'ClawTalk routing patch version')
...
assertIncludes(routingHelpers, 'TELEGRAM_AGENT_ROUTING_PATCH_VERSION = 16', 'Telegram routing patch version')
...
assertIncludes(routingHelpers, 'Configured primary execution model', 'Telegram routed agent model context')
...
assertIncludes(server, 'CLAWTALK_SMS_DELIVERY_RETRY_PATCH_VERSION = 1', 'ClawTalk SMS transient-delivery retry patch')
assertIncludes(server, 'sendClawTalkSmsWithRetry', 'ClawTalk SMS retry helper')
