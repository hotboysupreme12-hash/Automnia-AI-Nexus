...
const backupParent = path.resolve(process.env.DYSTOPAI_STATE_BACKUP_DIR || path.join(home, 'DystopAI Backups'))
...
try {
  if (command === 'backup') {
    ...
    const result = restoreStateBackup({
      backupRoot,
      targetRoot,
      force: /^(1|true|yes)$/i.test(String(process.env.DYSTOPAI_STATE_RESTORE_FORCE || '')),
    })
