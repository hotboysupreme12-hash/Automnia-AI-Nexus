...
assert.match(editor, /apiRequest<AgentResourceSavePayload>\(`\/api\/party\/resources\/\$\{encodeURIComponent\(savingAgentId\)\}/, 'agent editor file saves must use the canonical API client and captured agent identity')
assert.match(editor, /expectedRevision:resourceRevisionsRef/, 'agent file saves must send their loaded revision')
