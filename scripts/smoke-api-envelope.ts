...
assert.match(missionSlice, /apiFailure\(res, result\.status, result\.code, result\.message, result\.detail\)/, 'mission route service failures must use canonical error envelopes')
assert.match(missionStateService, /code: 'mission_invalid_state'/, 'mission stop must expose state-conflict codes')
assert.match(missionStateService, /code: 'mission_scheduler_failed'/, 'mission scheduler failures must be typed')
