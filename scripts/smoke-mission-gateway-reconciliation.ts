...
assert.match(missionRecoveryServiceSource, /client\.request\('sessions\.describe', \{ key: sessionKey \}, \{ timeoutMs \}\)/)
assert.match(missionRecoveryServiceSource, /gatewayErrorLooksNotFound\((?:error|detail)\)\s*\?\s*'missing'(?:\s+as const)?\s*:\s*'unavailable'/)
