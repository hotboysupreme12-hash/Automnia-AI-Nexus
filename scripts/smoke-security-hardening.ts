...
assert.match(electronMain, /DYSTOPAI_ELECTRON_E2E_ASSERT_NAVIGATION/, 'Electron must expose E2E-only navigation policy assertions')
assert.match(electronMain, /DYSTOPAI_ELECTRON_E2E_ASSERT_RENDERER_EXTERNALS/, 'Electron E2E must assert renderer-originated external navigation attempts')
assert.match(electronMain, /DYSTOPAI_ELECTRON_E2E_ASSERT_RENDERER_RECOVERY/, 'Electron E2E must assert renderer crash recovery')
assert.match(electronMain, /DYSTOPAI_ELECTRON_E2E_ASSERT_RENDERER_JOURNEY/, 'Electron E2E must exercise primary renderer workspaces')
assert.match(electronMain, /DYSTOPAI_ELECTRON_E2E_ASSERT_TRAY_BEHAVIOR/, 'Electron E2E must assert tray hide and restore behavior')
