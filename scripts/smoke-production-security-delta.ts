...
assert.match(tokenStorage, /storageWrite\(browserStorage\('local'\), token\)/, 'renderer auth tokens must persist across desktop launches')
assert.match(tokenStorage, /storageRemove\(browserStorage\('local'\)\)/, 'explicit logout must remove persistent renderer auth tokens')
...
assert.match(authenticatedFetch, /recoverDesktopControlCenterSession/, 'desktop auth must recover through the server-issued session adapter')
assert.match(desktopSessionRecovery, /bootstrapControlCenterSession/, 'desktop auth must request only the narrow session bootstrap capability')
assert.match(desktopSessionRecovery, /writeAuthToken\(token\)/, 'desktop auth must receive and store only a server-issued session token')
assert.doesNotMatch(desktopSessionRecovery, /get-control-center-token|getControlCenterToken/, 'desktop session recovery must never access the long-lived launch token')
...
assert.match(client, /readAuthToken/, 'canonical API client must use shared persistent token storage')
