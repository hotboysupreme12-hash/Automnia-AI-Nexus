...
assert.match(workflowSource, /DYSTOPAI_RELEASE_SIGNING_ENABLED/, 'CI must expose an explicit release-signing enable switch')
assert.match(workflowSource, /DYSTOPAI_RELEASE_REQUIRE_SIGNING/, 'CI must expose mandatory public-release signing validation')
assert.match(workflowSource, /public_release/, 'CI must allow manual public release validation runs')
assert.match(workflowSource, /refs\/tags\/v/, 'CI must require signing on version-tag release runs')
assert.match(workflowSource, /DYSTOPAI_RELEASE_SIGNING_PRIVATE_KEY_PEM/, 'CI must use the configured release-signing private-key secret')
assert.match(workflowSource, /npm run release:sign/, 'CI must be able to sign release evidence when enabled')
