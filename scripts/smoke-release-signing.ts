...
assert.match(signingSource, /AUTOMNIA_RELEASE_SIGNING_PRIVATE_KEY_FILE/, 'release signing must support Automnia private-key file input')
assert.match(signingSource, /AUTOMNIA_RELEASE_SIGNING_PRIVATE_KEY_PEM/, 'release signing must support Automnia private-key env input')
...
assert.match(publicReleaseSource, /AUTOMNIA_RELEASE_REQUIRE_SIGNING:\s*'0'/, 'early public-release CI must permit unsigned platform distribution')
assert.match(publicReleaseSource, /AUTOMNIA_SKIP_PLATFORM_SIGNING:\s*'1'/, 'early public-release CI must select unsigned platform packaging')
assert.match(publicReleaseSource, /AUTOMNIA_RELEASE_SIGNING_PRIVATE_KEY_PEM/, 'public-release CI must retain optional checksum-evidence signing')
assert.match(publicReleaseSource, /npm run release:sign/, 'public-release CI must retain optional release evidence signing')
