...
assert(!settingsActivityLog.includes('dui-activity-log__card'), 'Settings Logs should not render the removed activity timeline')
assert(!settingsActivityLog.includes('useGatewayActivityFeed'), 'Settings Logs should not poll the removed activity timeline')
...
assert(nexusShell.includes('<LiveOperationMonitor status={runtimeStatus}'), 'Monitor workspace should render LiveOperationMonitor with runtime status')
...
assert(liveMonitor.includes("const [tab, setTab] = useRememberedState<MonitorTab>('monitor-tab', 'gateway')"), 'Monitor should open on the Gateway runtime tab')
assert(nexusShell.includes('useRuntimeSummaryStatus(runtimeSummaryPollInterval)'), 'Monitor shell should subscribe to runtime summary status')
...
assert(liveMonitor.includes('<GatewayActivityPanel agentById={agentById} gatewayLogs={projectedGatewayLogs} />'), 'Unified Gateway activity should receive Gateway log-tail evidence')
