...
assert.match(validatorSource, /distribution-signing\.json/, 'release validation must support consumer distribution signing evidence for strict signed builds')
assert.match(validatorSource, /authenticode/, 'release validation must support Windows Authenticode evidence when strict platform signing is enabled')
