...
async function inspectRecruitMarkdownEditor(window) {
  const inspectScript = [
    ...
    "    let submitCount = 0",
    "    modal.querySelector('form').addEventListener('submit', () => { submitCount += 1 })",
    ...
    "    }",
    "    input.focus()",
    "    const editedContent = input.value + '\\nSmoke review: preserve my operating-file edits.\\n'",
    "    Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, 'value').set.call(input, editedContent)",
    "    input.dispatchEvent(new Event('input', { bubbles: true }))",
    "    await wait(120)",
    "    modal.querySelector('.dui-recruit-secondary').click()",
    "    await wait(120)",
    "    if (!await nextStep()) return { present: false, reason: 'recruit-return-to-files-failed' }",
    "    const editsPreserved = modal.querySelector('.dui-recruit-code-input')?.value === editedContent",
    "    const reviewRequiresExplicitRecruit = submitCount === 0 && modal.querySelector('.dui-recruit-wizard')?.getAttribute('data-step') === '4' && modal.querySelector('.dui-recruit-primary')?.textContent.includes('Recruit Agent')",
    "    modal.querySelector('.dui-recruit-code-input')?.focus()",
    "    await wait(120)",
    "    const currentInput = modal.querySelector('.dui-recruit-code-input')",
    "    const currentPreview = modal.querySelector('.dui-recruit-code-preview')",
    "    const currentEditor = modal.querySelector('.dui-recruit-code-editor')",
    "    const currentTab = modal.querySelector('.dui-recruit-file-tabs button[data-md-tone]')",
    "    const inputStyle = getComputedStyle(currentInput)",
    "    const previewStyle = getComputedStyle(currentPreview)",
    "    const editorStyle = getComputedStyle(currentEditor)",
    "    const beforeStyle = getComputedStyle(currentTab, '::before')",
    "    const icon = currentTab.querySelector('.dui-recruit-file-tab__icon')",
    "    const name = currentTab.querySelector('.dui-recruit-file-tab__name')",
    "    const type = currentTab.querySelector('.dui-recruit-file-tab__type')",
    ...
    "      tabGridTemplateColumns: getComputedStyle(currentTab).gridTemplateColumns,",
...
async function inspectViewport(viewport) {
  ...
  const ok = failures.length === 0
    ...
    && recruitMarkdown.reviewRequiresExplicitRecruit
    && recruitMarkdown.editsPreserved
