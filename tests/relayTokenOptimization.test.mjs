...
test('hosted relay keeps tool names and required schema fields but drops verbose schema metadata', () => {
  ...
  assert.ok(schema.properties.path.description.length <= 200)
})
...
test('hosted relay chooses a smaller automatic output budget and honors explicit bounded budgets', () => {
  ...
  assert.equal(resolveRelayOutputTokenBudget({}, { hasTools: true }).maxOutputTokens, 3_072)
  assert.equal(resolveRelayOutputTokenBudget({ reasoning_effort: 'high' }, { hasTools: true }).maxOutputTokens, 4_096)
  assert.deepEqual(resolveRelayOutputTokenBudget({ max_tokens: 20_000 }), { maxOutputTokens: 4_096, source: 'caller', explicit: true })
})
