...
test('credits-only policy exposes a bounded Automnia-hosted fallback chain', () => {
  ...
  assert.equal(AUTOMNIA_CREDITS_MODEL_ID, 'automnia-cloud/gemini-3.8-flash')
  assert.deepEqual(AUTOMNIA_CREDITS_MODEL_IDS, [AUTOMNIA_CREDITS_MODEL_ID, 'automnia-cloud/gemini-3.7-flash', ...AUTOMNIA_CREDITS_FALLBACK_MODEL_IDS])
  ...
  assert.equal(isAutomniaCreditsModelId('automnia-cloud/gemini-3.8-flash'), true)
  ...
  assert.deepEqual(
    filterCreditsOnlyModels([
      ...
      { id: 'automnia-cloud/gemini-3.8-flash', name: 'Automnia Prime' },
