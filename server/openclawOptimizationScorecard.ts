    ...
    items: [
      {
        ...
        status: 'implemented',
        priority: 'p0',
        evidence: 'POST /api/doctor/run executes the runtime Doctor checks.',
        nextAction: 'Keep Doctor checks small enough to run during interactive troubleshooting.',
      },
      {
        title: 'Read recent Doctor diagnostics',
        status: 'implemented',
        priority: 'p0',
        evidence: 'GET /api/doctor/recent returns the latest diagnostic summary.',
        nextAction: 'Show the most recent Doctor summary in runtime status UI.',
      },
      {
        title: 'Include Gateway health in Doctor checks',
        status: 'implemented',
        priority: 'p0',
        evidence: 'Doctor coverage includes Gateway health and chat readiness.',
        nextAction: 'Capture the last Gateway restart reason in the same payload.',
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        status: 'partial',
        priority: 'p1',
        evidence: 'Gateway restart controls exist; last restart reason is not yet first-class in health.',
        nextAction: 'Store and display last Gateway restart reason in health and Doctor diagnostics.',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        status: 'partial',
        priority: 'p1',
        evidence: 'Gateway plugin restart exists; plugin-specific repair flow needs a common contract.',
        nextAction: 'Add repair actions with bounded retries and ledger evidence.',
      },
      {
