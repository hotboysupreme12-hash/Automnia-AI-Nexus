...
export function registerMissionRoutes(app: Express, options: MissionRoutesOptions) {
  ...
  app.post('/api/missions/start', async (req, res) => {
    const schema = z.object({
      ...
      missionType: z.enum(['codeGeneration', 'planning', 'research', 'orchestration', 'memoryManagement']).optional(),
      collaborationMode: z.enum(['parallel', 'sequential', 'hierarchical', 'swarm', 'specialist']).optional(),
      ...
      agentCadenceSeconds: z.record(z.string().min(1), z.number().int().min(15).max(24 * 60 * 60)).optional(),
