...
async function runMode(writeMode) {
  ...
  try {
    ...
    if (writeMode === 'read_only') {
      ...
      assert.equal(provisioned.records[0]?.creditBalance, 200_000, 'Starter must grant 200,000 customer-facing credits')
      ...
      assert.equal(refilledOwner?.creditBalance, 10_200_000, '10M Shopify refill must add to the existing hosted-credit wallet')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 10_200_000, 'Duplicate Shopify delivery must not double-credit the wallet')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 10_300_000, '$1 Shopify refill must add 100,000 hosted credits')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 10_300_000, 'Duplicate $1 Shopify delivery must not double-credit the wallet')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 10_500_000, 'A quantity-2 100K refill must add 200,000 hosted credits')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 10_500_000, 'A duplicate quantity-2 delivery must not double-credit the wallet')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 30_500_000, 'A quantity-2 10M refill must add 20,000,000 hosted credits')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 30_500_000, 'A duplicate quantity-2 large refill must not double-credit the wallet')
      ...
      assert.equal(provisioned.records[0]?.usagePriority, 'automnia_only')
      assert.equal(provisioned.records[0]?.creditBalance, 30_900_000, 'Pro must add 400,000 credits to the existing wallet')
      ...
      assert.equal(upgradedProRecord?.creditBalance, 600_000, 'Starter credits must survive a Pro upgrade')
