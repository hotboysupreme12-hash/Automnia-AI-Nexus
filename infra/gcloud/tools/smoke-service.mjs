...
async function runMode(writeMode) {
  ...
  try {
    ...
    if (writeMode === 'read_only') {
      ...
      assert.equal(provisioned.records[0]?.creditBalance, 200, 'Starter must display 200 credits for its 200,000-token grant')
      ...
      assert.equal(refilledOwner?.creditBalance, 10_200, '10M Shopify refill must display as 10,000 credits and add to the existing wallet')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 10_200, 'Duplicate Shopify delivery must not double-credit the wallet')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 10_300, '$1 Shopify refill must add 100 credits')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 10_300, 'Duplicate $1 Shopify delivery must not double-credit the wallet')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 10_500, 'A quantity-2 100K refill must add 200 credits')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 10_500, 'A duplicate quantity-2 delivery must not double-credit the wallet')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 30_500, 'A quantity-2 10M refill must add 20,000 credits')
      ...
      provisioned = await fetch(`${baseUrl}/provisioned`, { headers: { Authorization: 'Bearer local-smoke-admin-value' } }).then((response) => response.json())
      assert.equal(provisioned.records.find((record) => record.email === 'owner@example.test')?.creditBalance, 30_500, 'A duplicate quantity-2 large refill must not double-credit the wallet')
      ...
      assert.equal(provisioned.records[0]?.usagePriority, 'automnia_only')
      assert.equal(provisioned.records[0]?.creditBalance, 30_900, 'Pro must add 400 credits to the existing wallet')
      ...
      assert.equal(upgradedProRecord?.creditBalance, 600, 'Starter credits must survive a Pro upgrade')
