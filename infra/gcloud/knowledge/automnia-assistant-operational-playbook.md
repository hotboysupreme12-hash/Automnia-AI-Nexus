...
The password is not a substitute for a provider API key. Hosted-credit requests still require a valid Automnia entitlement and are sent through the Automnia Cloud relay. Starter is hosted Automnia Subscription Relay access with 22,000 credits backed by 22,000,000 hosted tokens and does not include provider access. Pro includes 55,000 credits backed by 55,000,000 hosted tokens, complete advanced access, and eligible connected-provider routes. If the account screen shows a different entitlement, trust the account screen and do not infer a price.
