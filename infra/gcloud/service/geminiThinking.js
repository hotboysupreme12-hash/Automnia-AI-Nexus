 ...
 * thinking-off mode: Gemini 3.6 uses MINIMAL, while Gemini 3.7/3.8's lowest
...
export function geminiThinkingConfigFromOpenAiRequest(requestBody, model = 'gemini-3.6-flash') {
  ...
  const isGemini37Or38Flash = /(?:^|\/)gemini-3\.(?:7|8)-flash(?:$|[-@])/.test(normalizedModel);
  ...
  const nativeLevels = isGemini37Or38Flash ? GEMINI_37_NATIVE_THINKING_LEVELS : GEMINI_36_NATIVE_THINKING_LEVELS;
  const lowestLevel = isGemini37Or38Flash ? 'low' : 'minimal';

  const level = raw === 'off' || raw === 'none'
    ? lowestLevel
    : raw === 'xhigh' || raw === 'max' || raw === 'ultra'
      ? 'high'
      : raw === 'minimal' && isGemini37Or38Flash
        ? lowestLevel
